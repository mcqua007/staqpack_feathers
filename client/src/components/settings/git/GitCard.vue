<template lang="html">
 <div class="card-wrap col-sm-12 col-md-6 col-lg-4"><!-- Component Wrapper -->
  <div class="card">
    <div class="card-body">
    <span class="card-title git-title">{{ repoData.name }}<a href="data" target="_blank" class="pull-right"><i class="la la-external-link"></i></a></span><br />
    ...
    <div class="card-text">{{ shortDesc(repoData.description) }}</div>
      ...
      <button class="btn-secondary sync-button" data-index="key" @click="syncRepo(repoData)"><i class="la la-sync"></i></button>
    </div>
   </div>
  </div>
</template>
