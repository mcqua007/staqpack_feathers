<template lang="html">
  <div class="top-bar" :style="{backgroundColor: this.$store.getters.themeColor}">
    ...
    <button class="add-btn-menu c-white"  @click="toggleDropdown()" id="btnGroupDrop1" data-toggle='dropdown' aria-haspopup="true" aria-expanded="false" >
      <i :class="{'la':true, 'la-plus':true, 'active':(dropdownExpanded == true)}"></i></button>
     <div  v-if="dropdownExpanded" :class="{'dropdown-menu':true,'dropdown-menu-right': true, 'active':(dropdownExpanded == true)}" aria-labelledby="btnGroupDrop1" data-dropdown-task-id="">
       <button class="dropdown-item"   type="button" @click="toggleTaskForm()" ><i class="la la-plus margin-r-5"></i> New Task </button>
      <button class="dropdown-item"   type="button"  @click="toggleProjectForm()" > <i class="la la-plus margin-r-5"></i> New Project</button>
