...
<!-- eslint-disable -->
  <div v-bind:class="{'sidebar-nav':true, 'expanded':($store.getters.sideBarState == true)}">
   <nav class="text-left">
     <div class="root-nav-item" role="button" tabindex="0" @click="toggleUserMenu()"><i class="la la-user-circle"></i> {{ $store.getters.user.email}} </div>
      <div v-show="userMenuState" class="inner-menu" :style="{borderTop: '2px solid '+this.$store.getters.themeColor}">
        <ul>
          <li class="m-top-8" role="button"  @click="$store.dispatch('logout')"><i class="la la-sign-out-alt"></i> Logout</li>
    ...
    openAllTasks() {
      ...
      this.$store.dispatch("fetchAllTasks").then(() => {
        ...
        if (this.$store.getters.settingsState === true) {
          this.$store.commit(
            "toggleSettingsState",
            this.$store.getters.settingsState
          );
        }
        ...
        if (this.$store.getters.allTasksState === false) {
          this.$store.commit(
            "toggleAllTasksState",
            this.$store.getters.allTasksState
          );
        }
      });
    },
